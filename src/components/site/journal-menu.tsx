...
import type { JournalMenu } from "@/lib/nav-menu";
...
export default function JournalMenu({
  ...
  return (
    <div
      aria-hidden={!open}
      className={`border-b border-ink/10 bg-ivory shadow-[0_28px_60px_-34px_rgba(27,24,21,0.4)] transition-[opacity,transform] duration-[420ms] ease-lux ${
        open
    ...
    >
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-10 md:grid-cols-[190px_1fr] md:gap-14 md:px-10 md:py-12">
        {/* Categories */}
        <div>
          <p className="font-mono text-[0.6rem] uppercase tracking-[0.28em] text-ink/40">
            Categories
          </p>
              ...
              <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-ink/[0.05]">
                {post.image ? (
                  <Image
                    ...
                    className="object-cover transition-transform duration-[900ms] ease-lux group-hover:scale-[1.05]"
                  />
                ) : null}
              </div>
              <p className="mt-3 font-mono text-[0.58rem] uppercase tracking-[0.2em] text-ink/45 transition-colors duration-300 ease-lux group-hover:text-clay">
                {post.category}
              </p>
